...
from SRCNN.baseline import Net
import random
from torch.optim.lr_scheduler import CosineAnnealingWarmRestarts
from skimage.color import rgb2ycbcr


def load_x2_pretrained(model, pretrained_path):
    ckpnt_dict = torch.load(pretrained_path)
    exclude_weights = ['last_part.high_par.weight', 'last_part.low_par2.weight']
    ckpnt_dict = {k: v for k, v in ckpnt_dict.items() if k not in exclude_weights}

    model_dict = model.state_dict()
    model_dict.update(ckpnt_dict)
    model.load_state_dict(model_dict)

    return model

def val_psnr(model, val_path, scale, th, dilker, dilation, gray=True):  #set5
    images = sorted(os.listdir(val_path))   # 경로에서 이미지를 읽어오고
    avg_psnr = 0
    for image in images:
        ...
        try:
            h, w, c = img.shape
            img = rgb2ycbcr(img)[:, :, 0:1] # 3차원이면 ycbcr로 변환후 y값만 사용
        except:
            h, w = img.shape
            img = np.expand_dims(img, axis=-1)  # 2차원이면 한차원 늘려준다

        img = np.float64(img)/255
        h, w, c = img.shape


        orig = img[0:h - (h % scale), 0:w - (w % scale)]    # 다운스케일 가능하도록 이미지 자르고
        lr = imresize(orig, scalar_scale=1 / scale, method='bicubic')   # 바이큐빅
...
save_as = 'SRCNN/checkpoint/scale_x2/baseline.pth'
th = 0.01
dilation= True
r = 8
dilker = 3
...
model = Net(scale=train_scales[0], r= r)
model = model.float()

# pretrained_path = 'ASFSR/checkpoint/scale_x2/th_03_dilk3.pth'
# model = load_x2_pretrained(model, pretrained_path)
...
for itr_part in range(start_from, itr_parts+1):
    ...
    for step in range (1, n_steps+1):
        ...
        predict = model(lr_batch, th, dilker, dilation, cut)

        loss = mae(predict, hr_batch[:, :, cut:-cut,cut:-cut])
    ...
    with torch.no_grad():
        ...
        for scale in test_scales:
            avg_val_psnr = val_psnr(model, val_path, scale=scale, th=th, dilker=dilker, dilation=dilation)
